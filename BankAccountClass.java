...
public class BankAccountClass {
	...
	public void withdraw(double amount) throws InvalidAmountExceptions, InsufficientFundsExceptions {

		logger.info("Withdraw requested : ₱{} ", amount );

		if(amount < 0 ) {
				logger.error("ERROR : cannot withdraw amount entered : ₱{} ",amount);
				throw new InvalidAmountExceptions("ERROR : cannot withdraw amount entered ");
		}

	if(amount > balance ){
			logger.warn("WARNING : Insufficient funds, amount entered : ₱{} ; current balance is : Balance {}", amount,balance);
			throw  new InsufficientFundsExceptions("ERROR : Insufficient amount requested");
	}
	...
	public void deposit(double amount) throws  InvalidAmountExceptions {
		...
		if (amount <= 0){
				...
				throw new InvalidAmountExceptions("ERROR : Invalid ammount entered");
		}
